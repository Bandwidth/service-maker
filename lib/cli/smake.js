  ...
  .action(function(env, options) {
    ...
    request({
      ...
      if (!error && response.statusCode == 200) {
        ...
        console.log('The following instances are avaialble');
        Object.keys(isAvailable).forEach(function(type) {
          console.log(type, ':', isAvailable[type].length);
        });
        console.log('The following instances are unavaialble');
    ...
    request(url, function(error, response, body) {
      if (!error && response.statusCode == 201) {
        ...
        , url: instanceUrl
          ...
          if (!error && response.statusCode == 200) {
            body = JSON.parse(body)[0];
            console.log('Please connect to AWS with the following IP: ' + body.PublicIpAddress + '\n');
