...
exports.register = function (server, options, next) {
	...
	server.route({
		...
		handler : function (request, reply) {
			...
			.then(function (response) {
				reply(response);
				return awsAdapter.terminateInstances(id);
			})
			.then(function () {
				updatedInstance.state = "terminated";
				return instances.updateInstance(updatedInstance);
			})
			.catch (function (error) {
				switch (error.name) {
					...
					case ("InvalidInstanceID.Malformed") : {
						server.log([ "error", "rest", "InvalidInstanceID" ], "InstanceID entered incorrectly.");
						reply(
							Boom.badRequest("The InstanceId entered is not valid. Ensure it is of the form i-xxxxxx.")
						);
						break;
					}
					...
					default : {
						server.log([ "error", "rest", "DefaultError" ], error);
						reply(Boom.wrap(error));
					}
				}
			});
