...
var CacheAdapter    = require("../services/cacheAdapter");
...
exports.register = function (server, options, next) {
	...
	var cacheClient   = new Catbox.Client(CatboxMongoDb, catboxOptions);
	cacheClient.start(function (err) {
		var cache = new CacheAdapter(options.mapper);
	});
	var awsAdapter;
	...
	server.route({
		...
		handler : function (request, reply) {
			...
			cacheClient.get(key, function (err, cached) {
				if (err) {
					console.log("Error" + err);
				}
				else if (cached) {
					console.log("Cahed" + cached.item);
				}
				else {
					cacheClient.set(key, "hehe");
