...
function InstancePool(strategy) {
  ...
  if (!AWS.config.credentials) {
    throw new Error('Cannot create an instance pool; AWS credentials not configured.');
  }
  ...
  this.poolingStrategy = new _strategy();
  this.ec2 = ec2;
}

// Returns the name of the pooling strategy currently in use
InstancePool.prototype.strategy = function(callback) {
  return this.poolingStrategy.constructor.name;
};
...
InstancePool.create = function(type, _callback) {
  ...
  ec2.runInstances(params, function(err, data) {
    ...
    Tags.applyTags(POOL_TAGS, instanceId, function() {
      Tags.applyTags(SSHABLE_TAG, instanceId, callback(instanceId));
    });
...
// Encapsluate in function to make looping easier
function create(instanceType) {
  InstancePool.create(instanceType, function(instanceId) {
...
// as necessary.
InstancePool.prototype.initialize = function(server, _done) {
  function done() { if (_done) { _done(); } }
  ...
  this.getPoolInstances(function(currentInstances) {
    if (currentInstances === undefined) {
      if (server) { server.log('info', 'Did not find existing pool resources'); }
      ...
      requiredInstances.forEach(function(requiredInstance) {
        ...
        for (var i = 0; i < count; i++) {
          InstancePool.create(type, function(instanceId) {
            if (server) { server.log('info', 'Starting new ' + type + ' instance ' + instanceId); }
          });
        }
      });
      done();
    ...
    if (server) { server.log('info', 'Assimilating existing pool resources...'); }
    ...
    Object.keys(diff).forEach(function(instanceType) {
        ...
        currentInstances[instanceType].forEach(function(currentInstance) {
          if (destroyed++ < Math.abs(typeDiff)) {
            InstancePool.terminate(currentInstance.InstanceId, function(instanceId) {
              if (server) { server.log('info', 'Terminating ' + instanceType + ' instance ' + instanceId); }
            });
          }
        });
      }
      ...
      if (typeDiff > 0) {
        ...
        var totalRequired = created + typeDiff;
        for (; created < totalRequired; created++) {
          create(instanceType);
        }
      }
    ...
    if (server) { server.log('info', 'Instance pool started successfully!'); }
...
InstancePool.prototype.getInstance = function(instanceSpec, callback) {
  this.getPoolInstancesByType(instanceSpec.type, function(instances) {
    ...
    InstancePool.removeFromPool(toReturn, function() {
      self.poolingStrategy.notifyOfRemoval(instanceSpec.type, function(toCreate) {
