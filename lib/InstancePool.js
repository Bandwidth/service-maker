...
InstancePool.create = function(type, callback) {
  ...
  ec2.runInstances(params, function(err, data) {
    ...
    Tags.applyTags(POOL_TAGS, instanceId, function(instanceId) {
      if (callback) {
        callback(instanceId);
      }
    });
...
InstancePool.removeFromPool = function(instanceId, done) {
  Tags.removeTags(POOL_TAGS, instanceId, function() {
    if (done) { done(); }
  });
};
