...
InstancePool.applyTags = function(tags, instanceId, callback) {
  console.log('in apply tags with instanceid', instanceId);
  console.log('trying to apply tags', tags);
  var tries = 3;
...
InstancePool._applyTags = function(tags, instanceId, tries, callback) {
  ...
  console.log('before ec2.createTags')
  ec2.createTags(params, function(err) {
    console.log('in ec2.createTags');
    if (err) {
      callback(undefined, tries - 1);
    } else {
      console.log('in _applyTags with instanceId', instanceId, 'and tags', tags);
...
InstancePool.applyTtlTo = function(instanceId, timeToLive) {
  console.log('trying to apply TTL tags to', instanceId);
  ...
  InstancePool.applyTags(tags, instanceId, function(instanceId) {
    console.log('Applied TTL tags to instance', instanceId);
  });
};
...
InstancePool.prototype.getInstance = function(instance, callback) {
  this.getPoolInstancesByType(instance.type, function(instances) {
    ...
    if (instance.ttl !== undefined) {
      InstancePool.applyTtlTo(toReturn, instance.ttl);
    }

    InstancePool.removeFromPool(toReturn, function() {
      poolingStrategy.notifyOfRemoval(instance.type, function(toCreate) {
