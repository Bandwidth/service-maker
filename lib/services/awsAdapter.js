...
function AWSAdapter (options) {
	...
	this.beginPolling = function (result) {
		...
		var objectSchema = Joi.object().keys({
				...
				uri        : Joi.string().required().allow(null),
				revision   : Joi.number().required(),
				instanceID : Joi.string().required()
		});

		var validationResult = Joi.validate(result, objectSchema);
		if (validationResult.error) {
			console.log("Erroer1: " + validationResult.error);
			serverLog([ "error", "AWSAdapter", "beginPolling" ], validationResult.error);
			return Bluebird.reject(validationResult.error);
		}
		console.log("blassdf");
