...
function AWSAdapter (options) {
	...
	this.getPublicIPAddress = function (instanceID) {
		...
		.then(function (instance) {
			//console.log("MY Instance: " + JSON.stringify(instance));
	...
	this.beginPolling = function (result) {
		...
		.then(function () {
			return self.updatePollingStatus(result);
		})
		.catch(function (error) {
			return self.handlePollingErrors(result, error);
		});
	};
	...
	this.checkInstanceStatus = function (instance) {
		...
		.then(function (sshable) {
			if (sshable) {
				return self.updatePollingStatus(instance);
			}
			else {
				return instance;
			}
		...
		.catch(function (error) {
			return self.handlePollingErrors(instance, error);
		});
	};
}
